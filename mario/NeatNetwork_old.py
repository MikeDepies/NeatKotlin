...
def constructNetwork(nodes: List[NodeLocation], connections: List[ConnectionLocation], layerShapes: List[List[int]], bias: NodeLocation = None):
    ...
    connection = [np.zeros(layerShapes[1] + layerShapes[0]),
                ...
                np.zeros(layerShapes[1] + layerShapes[7]),
                np.zeros(layerShapes[2] + layerShapes[7]),
                np.zeros(layerShapes[3] + layerShapes[7]),
                np.zeros(layerShapes[4] + layerShapes[7]),
                ]
...
class ComputableNetwork:
    ...
    def compute(self):
        ...
        # layer 1
        v1: ndarray = (self.inputNdArray * self.connection[0]).sum((2, 3)) + (self.controller_input * self.connection[23]).sum((2, 3))
        self.values[1][..., 0] = v1 + self.connection[17]
        self.values[1][..., 1] = vectorizedRelu(v1)

        # layer 2
        v2: ndarray = (self.values[1][..., 1] * self.connection[1]).sum((2, 3))  + (self.controller_input * self.connection[24]).sum((2, 3))
        ...
        # layer 3
        v3: ndarray = (self.values[2][..., 1] * self.connection[4]).sum((2, 3))  + (self.controller_input * self.connection[25]).sum((2, 3))
        self.values[3][..., 0] = v3 + (self.values[3][..., 1] * self.connection[11]).sum((2, 3)) + self.connection[19]
        self.values[3][..., 1] = vectorizedRelu(self.values[3][..., 0])
        
        # layer 4
        v4: ndarray = (self.values[3][..., 1] * self.connection[5]).sum((2, 3))  + (self.controller_input * self.connection[26]).sum((2, 3))
        ...
        self.values[4][..., 1] = vectorizedRelu(self.values[4][..., 0])
        ...
        sum = v10  + vSelf  + self.connection[22]
        # print("=========")
        # print(v3)
