...
@Serializable
data class NeatModel(val nodes: List<NodeGeneModel>, val connections: List<ConnectionGeneModel>, val uuid: String)
...
fun NeatMutator.toModel() = NeatModel(nodes.map { it.toModel() }, connections.map { it.toModel() }, id.toString())
...
fun SpeciesScoreKeeper.toModel() =
    SpeciesScoreKeeperModel(this.speciesScoreMap.map { it.key.id to it.value.modelScore.toModel() }.toMap())
