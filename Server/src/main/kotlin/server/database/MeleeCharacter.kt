...
enum class MeleeCharacter {
    ...
    companion object {
        val map = MeleeCharacter.values().map { it.id to it }.toMap()
        fun forId(characterId : Int): MeleeCharacter {
            return map[characterId] ?: error("No melee character for id $characterId")
        }
    }
}
