...
def constructNetwork(nodes: List[NodeLocation], connections: List[ConnectionLocation], layerShapes: List[List[int]], bias: NodeLocation = None):
    ...
    connection : List[ndarray] = [np.zeros(layerShapes[1] + layerShapes[0]),
    ...
    for c in connections:
        ...
        try:
            if source.z == -2: # Bias only has target dimensions
                ...
                #     print(str(source) + " to " + str(target) + " = " + str(connectionIndex) + "connection Length: " + str(len(connection)) + "  weight: " + str(c.weight))
                connection[connectionIndex][target.y, target.x, source.y, source.x ] = c.weight
        except:
            print(str(source) + " to " + str(target) + " = " + str(connectionIndex) + "connection Length: " + str(len(connection)) + "  shape: " + str(connection[connectionIndex].shape))
...
class ComputableNetwork:
    ...
    def input(self, input: ndarray):
        ...
        self.values[0][...,0] = input
        self.values[0][...,1] = input
    ...
    def compute(self):
        ...
        # layer 1
        v1: ndarray = (self.values[0][..., 0] * self.connection[0]).sum((2, 3))
        self.values[1][..., 0] = v1 + self.connection[17]
        self.values[1][..., 1] = vectorizedSigmoidal(v1)
        # print(self.values[0][0, 415, 0])
        # for i in range(0, 22):
        #     print(str(i) + ": " + str(self.connection[i].max()))
        # print(v1.max())
        ...
        # + v6 + v7 + v8 + v9 + vSelf
        sum = (v10 + self.connection[22] + v6 + v7 + v8 + v9 + vSelf) 
        # print("=========")
        # print(v3)
