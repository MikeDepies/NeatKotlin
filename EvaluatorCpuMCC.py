...
class EvaluatorCpuMCC:
    ...
    def player_ground_move(self, game_state: GameState) -> bool:
        player: PlayerState = game_state.players[self.player_index]
        return not self.frame_data.is_roll(player.character, player.action) and not self.frame_data.is_bmove(player.character, player.action) and not self.frame_data.is_attack(player.character, player.action) and player.speed_ground_x_self != 0
