...
def startConsole(pr : cProfile.Profile):
...
def console_loop(pr : cProfile.Profile):
    # Main loop
    # if (Session.network0 != None and Session.network1 == None):
    # log = melee.Logger()
    i =0
    console, controller, controller_opponent, args, log = startConsole(pr)
    ...
    model_helper = ModelHelper(ai_controller_id, "192.168.0.139")
            ...
            if network is None or evaluator.is_finished(game_state):
                ...
                if model_index < len(model_list):
                    ...
                    if result.model_available:
                        ...
                        evaluator = Evaluator(player_index, opponent_index, 5, 45)
                        # break
