...
class Evaluator:
    ...
    def __init__(self, player: int, opponent: int, model_helper: ModelHelper, attack_timer: int = 5, max_timer: int = 60, action_limit: int = 5,  logger: melee.Logger = None) -> None:
    ...
    def is_on_stage(self, game_state: GameState, player: PlayerState) -> bool:
        ...
        # print(str(abs(player.position.x)) + " < " + str(right_edge_distance) + ": " + str(abs(player.position.x) < right_edge_distance))
    ...
    def capture_action(self, player: PlayerState):
        if player.action.value in self.player_previous_actions:
            return False
        else:
    ...
    def is_melee_cpu_stuck(self, game_state: GameState) -> bool:
        if not self.previous_frame:
            return False
        opponent_last: PlayerState = self.previous_frame.players[self.opponent_index]
        opponent: PlayerState = game_state.players[self.opponent_index]
        return opponent_last.position.x == opponent.position.x 
    ...
    def evaluate_frame(self, game_state: GameState) -> None:
        ...
        if self.last_x is None or game_state.frame < 0:
            ...
            if self.previous_frame and self.previous_frame.players[self.player_index].action != player.action:
                ...
                #     print("offstage: " + str(player.action))
                    self.recovery_actions.append(player.action.value)
            if self.player_lost_stock(game_state):
                
                Process(target=self.model_helper.updateDeath, args=(), daemon=True).start()
                self.player_died = True

            if self.opponent_lost_stock(game_state) and self.opponent_knocked:
                print("opponent lost stock")
                Process(target=self.model_helper.updateKill, args=(), daemon=True).start()
