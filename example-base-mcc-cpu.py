    ...
    while True:
        ...
        if game_state.menu_state in [melee.Menu.IN_GAME, melee.Menu.SUDDEN_DEATH]:
            ...
            if not (model_handler.network == None):
                if (score.deaths >= cpu_gene.deaths or score.total_damage_taken >= cpu_gene.damage_taken or score.total_frames_alive / 60 > max(1, .5 + cpu_gene.kills*.5) * (20 + cpu_gene.level * 2)):
                    mc_satisfy = False
